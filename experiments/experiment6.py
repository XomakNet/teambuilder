...
def experiment6(clustering_tool_type, clusters_number, input_data_file_name, lists_count):
    result_clusters = []
    is_all_clustered = False
    reader = DataReader(input_data_file_name)
    clustered_users = []
    max_cluster_size = ceil(len(cu.get_not_clustered_users_set(reader, clustered_users)) / clusters_number)

    while not is_all_clustered:

        # Get clusterings by lists
        clustering_tool = cu.ClusteringTools.build_clustering_tool(clusters_number, clustering_tool_type)
...
CLUSTERS_COUNT = 6
LISTS_COUNT = 2
INPUT_DATA_FILENAME = "../data/ms-sne_names.json"
...
if __name__ == '__main__':
    ...
    for i in range(0, REPEATS_COUNT):
        experiment_result = experiment6(CLUSTERING_TOOL_TYPE, CLUSTERS_COUNT, INPUT_DATA_FILENAME, LISTS_COUNT)
