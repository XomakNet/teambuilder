...
function Vizualizer(panelId, canvasId, data) {
    ...
    var listsToShow = [];
    var showDesires = true;
    var showInterClusterConnections = true;
    var sys;
        ...
        for(var pairIndex in data.pairDistances) {
            ...
            for(var listIndex in pair.distances) {
                ...
                if(showNotSignificant || _isSignificant(listDistance.distance)) {
                    _addEdge(pair.user1, pair.user2, {"type": "list", "listId": listDistance.id, "distance": listDistance.distance});
                }
...
function Renderer(canvas, visualizer) {
    ...
    {
        var label = node.data.name ? node.data.name : String(node.data.id);
